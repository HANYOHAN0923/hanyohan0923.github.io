...
- `skipTutorial()`은 듀토리얼을 스킵했을 때 SharedPreferences에 isFirstRun: false(<KEY>:<VALUE>를 저장한다. 따라서 앱을 재실행했을 때 main.dart의 firstRun이 false의 값을 갖기 때문에 가이드 스크린이 아닌 홈 스크린이 출력된다. 마지막으로 이 함수는 SharedPreferences를 사용하기 때문에 비동기 처리를 한다.
```dart
import 'package:flutter/material.dart';
